...
function A(e, i2, o2, a2, c2, s2) {
  if ("production" !== process.env.NODE_ENV && c2 === o2 && n(5), r(c2)) {
    var v2 = M(e, c2, s2 && i2 && 3 !== i2.i && !u(i2.R, a2) ? s2.concat(a2) : void 0);
    if (f(o2, a2, v2), !r(v2))
      return;
    e.m = false;
  }
...
const remapProperties = (obj, propertyMap, deleteRemappedProperties) => produce(obj, (draft) => {
  for (const [k2, v2] of Object.entries(propertyMap)) {
    draft[v2] = draft[k2];
...
const transformQuery = (query, options = {}) => {
  const {
    ruleProcessor = (r2) => r2,
  ...
  const processGroup = (rg) => ({
    ...
    rules: rg.rules.map((r2, idx) => {
      if (typeof r2 === "string") {
        return combinatorMap[r2] ?? r2;
      } else if ("rules" in r2) {
        return processGroup({ ...r2, path: [...rg.path, idx] });
      }
      return ruleProcessor(
        remapProperties(
          {
            ...{ ...r2, path: [...rg.path, idx] },
            operator: operatorMap[r2.operator] ?? r2.operator
          },
          propertyMap,
          deleteRemappedProperties
        )
